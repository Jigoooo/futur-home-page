...
- 아이콘은 반복되는 색상 박스보다 선 아이콘 자체가 먼저 보이게 한다.
...
- 카드 내부의 창, 노드, 막대 등 장식 전용 도형을 모두 제거한다.
...
- hover 시 경계선과 상단 hairline 색상만 전환하고, 텍스트 또는 아이콘은 최대 2px 이내에서 반응한다.
- 그림자 확대, 회전, 카드 상승 효과는 사용하지 않는다.
